=begin
#CyberSource Merged Spec

#All CyberSource API specs merged together. These are available at https://developer.cybersource.com/api/reference/api-reference.html

OpenAPI spec version: 0.0.1

Generated by: https://github.com/swagger-api/swagger-codegen.git
Swagger Codegen version: 2.4.38
=end

require 'date'

module CyberSource
  # Assurance data. Contains identity verification details that prove the consumer or device has been authenticated before the payment operation.
  class Acpv1tokensAssuranceData
    # Optional. Type of the verification data.   Possible values:   - `CARDHOLDER` (Default)   - `DEVICE` 
    attr_accessor :verification_type

    # Optional. Entity performing the verification.   Possible value:     - `10` - VISA (Default) 
    attr_accessor :verification_entity

    # Optional. Event where the verification occurred.   Possible values:     - `01` - Payment transaction   - `02` - Add card/Card enrollment   - `03` - Profile access   - `04` - Account verification 
    attr_accessor :verification_events

    # Required. Method of the verification.   Possible values:     - `02` - App-based authentication   - `04` - One-time passcode   - `21` - Visa Token Service step-up: Device binding   - `22` - Visa Token Service step-up: Cardholder verification   - `23` - FIDO2 
    attr_accessor :verification_method

    # Required. Result of the verification.   Possible values:     - `01` - Verified   - `02` - Not Verified   - `03` - Not performed   - `04` - Not required   - `21` - Not allowed 
    attr_accessor :verification_results

    # Required. Date and time the verification occurred. UTC time in Unix epoch format.
    attr_accessor :verification_timestamp

    attr_accessor :authentication_context

    attr_accessor :authenticated_identities

    # Additional data related to assurance.
    attr_accessor :additional_data

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'verification_type' => :'verificationType',
        :'verification_entity' => :'verificationEntity',
        :'verification_events' => :'verificationEvents',
        :'verification_method' => :'verificationMethod',
        :'verification_results' => :'verificationResults',
        :'verification_timestamp' => :'verificationTimestamp',
        :'authentication_context' => :'authenticationContext',
        :'authenticated_identities' => :'authenticatedIdentities',
        :'additional_data' => :'additionalData'
      }
    end

    # Attribute mapping from JSON key to ruby-style variable name.
    def self.json_map
      {
        :'verification_type' => :'verification_type',
        :'verification_entity' => :'verification_entity',
        :'verification_events' => :'verification_events',
        :'verification_method' => :'verification_method',
        :'verification_results' => :'verification_results',
        :'verification_timestamp' => :'verification_timestamp',
        :'authentication_context' => :'authentication_context',
        :'authenticated_identities' => :'authenticated_identities',
        :'additional_data' => :'additional_data'
      }
    end

    # Attribute type mapping.
    def self.swagger_types
      {
        :'verification_type' => :'String',
        :'verification_entity' => :'String',
        :'verification_events' => :'Array<String>',
        :'verification_method' => :'String',
        :'verification_results' => :'String',
        :'verification_timestamp' => :'String',
        :'authentication_context' => :'Acpv1tokensAuthenticationContext',
        :'authenticated_identities' => :'Acpv1tokensAuthenticatedIdentities',
        :'additional_data' => :'String'
      }
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      # convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h| h[k.to_sym] = v }

      if attributes.has_key?(:'verificationType')
        self.verification_type = attributes[:'verificationType']
      end

      if attributes.has_key?(:'verificationEntity')
        self.verification_entity = attributes[:'verificationEntity']
      end

      if attributes.has_key?(:'verificationEvents')
        if (value = attributes[:'verificationEvents']).is_a?(Array)
          self.verification_events = value
        end
      end

      if attributes.has_key?(:'verificationMethod')
        self.verification_method = attributes[:'verificationMethod']
      end

      if attributes.has_key?(:'verificationResults')
        self.verification_results = attributes[:'verificationResults']
      end

      if attributes.has_key?(:'verificationTimestamp')
        self.verification_timestamp = attributes[:'verificationTimestamp']
      end

      if attributes.has_key?(:'authenticationContext')
        self.authentication_context = attributes[:'authenticationContext']
      end

      if attributes.has_key?(:'authenticatedIdentities')
        self.authenticated_identities = attributes[:'authenticatedIdentities']
      end

      if attributes.has_key?(:'additionalData')
        self.additional_data = attributes[:'additionalData']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array.new
      if @verification_method.nil?
        invalid_properties.push('invalid value for "verification_method", verification_method cannot be nil.')
      end

      if @verification_results.nil?
        invalid_properties.push('invalid value for "verification_results", verification_results cannot be nil.')
      end

      if @verification_timestamp.nil?
        invalid_properties.push('invalid value for "verification_timestamp", verification_timestamp cannot be nil.')
      end

      #if @verification_timestamp !~ Regexp.new(/(?!^[*.,'#_\/-]+$)(?!.*\\.\/.*)^.+$/)
        #invalid_properties.push('invalid value for "verification_timestamp", must conform to the pattern /(?!^[*.,'#_\/-]+$)(?!.*\\.\/.*)^.+$/.')
      #end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      return false if @verification_method.nil?
      return false if @verification_results.nil?
      return false if @verification_timestamp.nil?
      #return false if @verification_timestamp !~ Regexp.new(/(?!^[*.,'#_\/-]+$)(?!.*\\.\/.*)^.+$/)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] verification_timestamp Value to be assigned
    def verification_timestamp=(verification_timestamp)
      #if verification_timestamp.nil?
        #fail ArgumentError, 'verification_timestamp cannot be nil'
      #end

      #if verification_timestamp !~ Regexp.new(/(?!^[*.,'#_\/-]+$)(?!.*\\.\/.*)^.+$/)
        #fail ArgumentError, 'invalid value for "verification_timestamp", must conform to the pattern /(?!^[*.,'#_\/-]+$)(?!.*\\.\/.*)^.+$/.'
      #end

      @verification_timestamp = verification_timestamp
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          verification_type == o.verification_type &&
          verification_entity == o.verification_entity &&
          verification_events == o.verification_events &&
          verification_method == o.verification_method &&
          verification_results == o.verification_results &&
          verification_timestamp == o.verification_timestamp &&
          authentication_context == o.authentication_context &&
          authenticated_identities == o.authenticated_identities &&
          additional_data == o.additional_data
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Fixnum] Hash code
    def hash
      [verification_type, verification_entity, verification_events, verification_method, verification_results, verification_timestamp, authentication_context, authenticated_identities, additional_data].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.swagger_types.each_pair do |key, type|
        if type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{self.class.json_map[key]}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{self.class.json_map[key]}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end # or else data not found in attributes(hash), not an issue as the data can be optional
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :DateTime
        DateTime.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :BOOLEAN
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        temp_model = CyberSource.const_get(type).new
        temp_model.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        next if value.nil?
        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end
  end
end
