...
module CyberSource
  class Ptsv1pullfundstransferProcessingInformationFundingOptionsInitiatorOriginatorInitiatedTransaction
    ...
    def list_invalid_properties
      ...
      #if !@reason.nil? && @reason !~ Regexp.new(/^(\\s{0,1}|.{1})$/)
        #invalid_properties.push('invalid value for "reason", must conform to the pattern /^(\\s{0,1}|.{1})$/.')
      #end
    ...
    def valid?
      #return false if !@reason.nil? && @reason !~ Regexp.new(/^(\\s{0,1}|.{1})$/)
      true
    end
    ...
    def reason=(reason)
      #if !reason.nil? && reason !~ Regexp.new(/^(\\s{0,1}|.{1})$/)
        #fail ArgumentError, 'invalid value for "reason", must conform to the pattern /^(\\s{0,1}|.{1})$/.'
      #end
    ...
    def hash
      [original_transaction_id, reason].hash
    end
