...
module CyberSource
  class RetrievalSummariesApi
    ...
    def get_retrieval_summary(start_time, end_time, opts = {})
      data, status_code, headers = get_retrieval_summary_with_http_info(start_time, end_time, opts)
    ...
    def get_retrieval_summary_with_http_info(start_time, end_time, opts = {})
      ...
      if @api_client.config.client_side_validation && start_time.nil?
        fail ArgumentError, "Missing the required parameter 'start_time' when calling RetrievalSummariesApi.get_retrieval_summary"
      end
      # verify the required parameter 'end_time' is set
      if @api_client.config.client_side_validation && end_time.nil?
        fail ArgumentError, "Missing the required parameter 'end_time' when calling RetrievalSummariesApi.get_retrieval_summary"
      end
      if @api_client.config.client_side_validation && !opts[:'organization_id'].nil? && opts[:'organization_id'].to_s.length > 32
        fail ArgumentError, 'invalid value for "opts[:"organization_id"]" when calling RetrievalSummariesApi.get_retrieval_summary, the character length must be smaller than or equal to 32.'
      end

      if @api_client.config.client_side_validation && !opts[:'organization_id'].nil? && opts[:'organization_id'].to_s.length < 1
        fail ArgumentError, 'invalid value for "opts[:"organization_id"]" when calling RetrievalSummariesApi.get_retrieval_summary, the character length must be great than or equal to 1.'
      end

      #if @api_client.config.client_side_validation && !opts[:'organization_id'].nil? && opts[:'organization_id'] !~ Regexp.new(/[a-zA-Z0-9-_]+/)
        #fail ArgumentError, "invalid value for 'opts[:\"organization_id\"]' when calling RetrievalSummariesApi.get_retrieval_summary, must conform to the pattern /[a-zA-Z0-9-_]+/."
      #end

      # resource path
      local_var_path = 'reporting/v3/retrieval-summaries'
