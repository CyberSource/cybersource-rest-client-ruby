# CyberSource::InlineResponse4041

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**submit_time_utc** | **String** | The time the response was submitted | [optional] 
**status** | **Integer** | The status code of the response | [optional] 
**reason** | **String** | The reason for the response | [optional] 
**message** | **String** | The message of the response | [optional] 
**details** | [**Array&lt;InlineResponse4041Details&gt;**](InlineResponse4041Details.md) | The details of the validation error | [optional] 


